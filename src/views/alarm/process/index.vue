...
import {
  ...
  updateProcessAlarmRequest,
  deleteProcessAlarmRequest
} from "@/api/alarm/process/index";
import { ProTableInstance, ColumnProps } from "@/components/ProTable/interface";
import { Delete, EditPen, View } from "@element-plus/icons-vue";
import { ElMessage } from "element-plus";
import alarmDrawer from "./alarmDrawer.vue";
import { useHandleData } from "@/hooks/useHandleData";
import { ProcessAlarm } from "@/api/alarm/process/type";
...
const openDrawer = (title: string, row: Partial<ProcessAlarm.AlarmItem> = {}) => {
  const params = {
    title,
    isView: title === "查看",
    row: { ...row },
    api: updateProcessAlarmRequest,
    getTableList: proTable.value?.getTableList
  };
  drawerRef.value?.acceptParams(params);
};

const updateAlarm = async (row: ProcessAlarm.AlarmItem) => {
  ...
  const response: BaseUpdateResponse = await updateProcessAlarmRequest(row);
...
// 删除告警信息
const deletePlugin = async (params: ProcessAlarm.AlarmItem) => {
  await useHandleData(deleteProcessAlarmRequest, params.id, `删除【${params.task_name}】告警`);
...
const dataCallback = (data: ProcessAlarm.AlarmResponse) => {
  return {
    list: data.list,
    total: data.total,
...
const getTableList = (params: ProcessAlarm.AlarmFilter) => {
  let newParams = JSON.parse(JSON.stringify(params));
  return getProcessAlarmRequest(newParams);
};

const columns = reactive<ColumnProps<ProcessAlarm.AlarmItem>[]>([
