...
import { getNodeMenuRequest } from "@/api/graph/graphData/index";
import { getTaskNodeRequest, getGraphNodeDataRequest } from "@/api/graph/graphData/index";
...
import { GraphData } from "@/api/graph/graphData/type";
...
const useGraphStore = defineStore("GraphStore", {
  state: (): FlowStoreType => {
    return {
      ...
      nodeMenu: [],
      // 节点配置
      nodeConfig: {} as GraphData.NodeConfig,
      // 节点参数
      nodeParam: [],
      // 节点结果
      nodeResult: [],
  ...
  actions: {
    ...
    async getTaskNode(taskNode: taskNodeType) {
      let response: BaseResponse<GraphData.TaskNode> = await getTaskNodeRequest(taskNode);
      if (response.code == 200) {
